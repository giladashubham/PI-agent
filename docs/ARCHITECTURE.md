...
Session UX surface: status/footer/banner/input-editor controls.
...
- `question-first-plan-mode.ts` — orchestration layer: `/plan on|off` toggling, state restore/persist, prompt injection, and safety hooks
- `plan/ask-questions-tool.ts` — reusable `ask_questions` tool (always available in normal + plan mode)
- `plan/tool-sets.ts` — tool-set shaping for normal mode vs plan mode (`setActiveTools` helpers)
- `plan/plan-prompts.ts` — plan-mode prompt snippet and plan-mode tool whitelist
- `plan/bash-safety.ts` — read-only bash allowlist/denylist for plan mode
- `plan/plan-config.ts` — optional model/thinking profile apply/restore for `/plan on|off`
...
No automatic plan artifact is generated by the mode extension. If requested, plans can be written manually by the agent to user-specified paths using normal file tools.
